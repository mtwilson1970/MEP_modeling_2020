...
%
% The conf file needs editing too, with the various parameters

close all; clear

%%%%%%%%%%% Input here %%%%%%%%%%%%%%%%%%%%%%%%%

%What kind of plot do we want? Once option is tru
ISIcurve=false; %plot against interstimulus interval for pairedpulse
my_values_ISI=[2000 1000 500 333 250 200 143 100 67 50 33 20 10 5 3 2];  %time intervals for ISI

ampcurve=true;  %plot against amplitude of stimulation for single pulse
my_values_amp=[500 600 700 800 900 1000 1100 1200 1300 1400]; %coarse variation
%my_values_amp=[450 475 500 525 550 575 600 625 650 675 700 725 750 775 800 850 900 950 1000 1100 1200 1300 1400 1500 1600];  %fine detail


gababcurve=false; %plot against length of GABAB timescale for paired pulse
MVCcurve=false; %Plot against percent MVC for single pulse Need to set other paras e.g. Burst number correctly in config template file


%What is the name of the configuration file to read parameters from?
template_to_read_from='marcus_template_june2019.conf';  %p7208. Paired pulse curve

%Do wwe want to save the output? To what file?
...
my_values = my_values_ISI;  %set the x axis to the specified ISI input values
...
if(ampcurve)
    my_string = 'Amplitude:';
 my_values=my_values_amp;  %set the x-axis to the specified amplitude input values
